...
import { BiCode, BiStar, BiTimer, BiTrendingUp, BiTrophy } from "react-icons/bi";
...
function ProfileHeader({ userSnippetStats, userData, user }: ProfileHeaderProps) {
  const starredSnippets = useQuery(api.snippet.getStarredSnippets);
  const STATS = [
    {
      label: "Code Executions",
      value: userSnippetStats?.totalExecutions ?? 0,
      icon: BsActivity,
      ...
      metric: {
        label: "Last 24h",
        value: userSnippetStats?.last24Hours ?? 0,
        icon: BiTimer,
      },
    },
    {
      ...
      metric: {
        label: "Most starred",
        value: userSnippetStats?.mostStarredLanguage ?? "N/A",
        icon: BiTrophy,
      },
    },
    {
      label: "Languages Used",
      value: userSnippetStats?.languageCount ?? 0,
      icon: BiCode,
      ...
      metric: {
        label: "Most used",
        value: userSnippetStats?.favouriteLanguage ?? "N/A",
        icon: BiTrendingUp,
      },
    },
  ];
