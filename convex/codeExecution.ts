...
export const getUserSnippetStats = query({
    ...
    handler : async(ctx,args) => {
        const executions =await ctx.db.query("codeExecution")
        .withIndex("by_user_id")
        .filter(q => q.eq(q.field("userId"),args.userId))
        .collect();
        ...
        const snippets = await Promise.all(starredSnippets.map(snippet => ctx.db.get(snippet.snippetId)));

        const starredLanguage = snippets.filter(Boolean).reduce(
            (acc,curr) => {
                if(curr?.language) {
        ...
        const favouriteLanguage = languages.length ? languages.reduce((a,b) => (languageStats[a] > languageStats[b] ? a : b))
        : "N/A";
