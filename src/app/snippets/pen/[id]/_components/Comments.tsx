...
import CommentForm from '@/app/snippets/_components/CommentForm';

function Comments({penId} : {penId : Id<"codepens">}) {
    const {user} = useUser();
    const [isSubmiting,setIsSubmiting] = useState(false);
    const [isDeleting, setIsDeleting] = useState(false);

    const comments = useQuery(api.codepens.getComment,{penId});
    const addComment = useMutation(api.codepens.addComment);
    const deleteComment = useMutation(api.codepens.deleteComment);
    ...
    const handleDelete = async(commentId : Id<"penComments">) => {
        setIsDeleting(true);
